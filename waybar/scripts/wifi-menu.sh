...
if ! command -v wofi &> /dev/null; then
  echo "wofi command not found. Please install it to use this script."
  exit 1
fi

# Enable pipefail option to handle broken pipe errors
set -o pipefail
...
# Prompt for password
get_password() {
  wofi --dmenu --password --conf="${config}" --prompt="Enter password" || pkill -x wofi
}
...
while true; do
  ...
  *"enabled"*)
    selected_option=$(echo "$options"$'\n'"$(wifi_list)" |
      wofi --dmenu --conf="${config}" --prompt="Select Wi-Fi" || pkill -x wofi)
    ;;
  *"disabled"*)
    selected_option=$(echo "$option_disabled" |
      wofi --dmenu --conf="${config}" --prompt="Enable Wi-Fi" || pkill -x wofi)
    ;;
  esac
  ...
  "Manual Entry")
    # Prompt for SSID
    manual_ssid=$(wofi --dmenu --conf="${config}" --prompt="Enter SSID" || pkill -x wofi)
