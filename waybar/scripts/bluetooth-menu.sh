...
# Wofi config
config="$HOME/.config/wofi/bluetooth-menu.css"
...
while true; do
  ...
  if [[ "$bluetooth_status" == "yes" ]]; then
    selected_option=$(echo -e "$options" | wofi --dmenu --insensitive --style "${config}" --prompt " " || pkill -x wofi)
  else
    selected_option=$(echo -e "$option" | wofi --dmenu --insensitive --style "${config}" --prompt " " || pkill -x wofi)
  fi
